# blocklist.de Blacklist Downloader r1bCL.09 for RouterOS v7
    ...
    :if ($sizeList!=0) do={
        ...
        :for i from=0 to=($endList) do={
            :if ([:toip ($addrList->$i)]) do={
                :if ($getIPv4) do={[:do {/ip firewall address-list add list=$listName address=($addrList->$i) comment=("Provided by ".$svcType) timeout=$timeout; :set $count4 ($count4+1)} on-error={}]}
            }
            :if ([:toip6 ($addrList->$i)]) do={
                :if ($getIPv6) do={[:do {/ipv6 firewall address-list add list=$listName address=($addrList->$i) comment=("Provided by ".$svcType) timeout=$timeout; :set $count6 ($count6+1)} on-error={}]}
            }
        }
